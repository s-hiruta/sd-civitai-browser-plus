...
def selected_to_queue(model_list, subfolder, download_start, create_json, current_html):
    ...
    items_by_id = {int(item['id']): item for item in gl.json_data['items']}
    is_windows = platform.system() == "Windows"

    for model_string in model_list:
        model_name, model_id = _api.extract_model_info(model_string)
        item = items_by_id.get(int(model_id))
        ...
        if not files:
            continue  # Skip if no files available

        primary_file = next((file for file in files if file.get('primary', False)), files[0])
        ...
        if default_subfolder != "None":
            default_subfolder = _file.convertCustomFolder(
                default_subfolder, output_basemodel, nsfw, model_uploader, model_name, model_id, version_name, version_id
            )

        if subfolder and subfolder not in ("None", "Only available if the selected files are of the same model type"):
            from_batch = False
            if is_windows:
            ...
            from_batch = True
            install_path = model_folder + default_subfolder if default_subfolder != "None" else model_folder
    ...
    return (
        ...
        gr.Button.update(interactive=(len(gl.download_queue) > 1), visible=True),  # Cancel All Button
